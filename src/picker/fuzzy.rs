//! Fuzzy matching wrapper over `nucleo-matcher`, ported from the
//! original zellij-zextract plugin's `crates/zextract/src/fuzzy.rs`
//! near verbatim — host-agnostic, no changes needed for the Herdr port.
//!
//! Smart-case: query containing any uppercase char → case-sensitive match.
//! Otherwise case-insensitive. Empty query returns all items in their
//! input order with empty indices.
...
impl FuzzyEngine {
    ...
    /// Variant of `filter_with_bonus` with a zero bonus for every item.
    #[cfg(test)]
    pub fn filter(&mut self, query: &str, items: &[&str]) -> Vec<ScoredMatch> {
        self.filter_with_bonus(query, items, |_| 0)
    }

    /// Filter/score `items` against `query`, adding a per-item bonus to
    /// the nucleo score (used by the picker to bias type-priority when
    /// fuzzy scores are close).
